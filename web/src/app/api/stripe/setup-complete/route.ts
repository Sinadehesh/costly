 ...
 * against Stripe server-side — the client's word counts for nothing, and the
 * user comes from the session cookie rather than the body (otherwise anyone
 * could attach a payment method to another account).
 */
export async function POST(req: Request) {
  ...
  const { setupIntentId } = bodySchema.parse(await req.json());
